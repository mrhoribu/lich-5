    ...
    "
      ...
      def self.resolve_channel_ref(channel)
        ...
        when :beta
          ...
          stable_major, stable_minor = major_minor_from(stable_tag)
          env = ENV['LICH_BETA_REF']
          return env unless env.nil? || env.empty?
          tag = latest_prerelease_tag_greater_than(stable_major, stable_minor)
          return tag if tag
          branch = latest_prefixed_branch_greater_than(BETA_BRANCH_PREFIX, stable_major, stable_minor)
      ...
      def self.latest_prerelease_tag_greater_than(stable_major, stable_minor)
        ...
        candidates = prereleases.select do |r|
          maj, min = major_minor_from(r['tag_name'])
      ...
      #
      # Extract major and minor version numbers from version string
      #
