...
describe ActiveSpell do
  context "updates spell information" do
    it "queries XMLData" do
      ...
      expect(ActiveSpell.instance_methods.update_spell_names).to eq(["Bravery", "Heroism", "Elemental Defense I", "Elemental Defense II", "Elemental Defense III", "Elemental Targeting", "Elemental Barrier", "Thurfel's Ward", "Elemental Deflection", "Elemental Bias", "Strength", "Elemental Focus", "Mage Armor", "Haste", "Temporal Reversion", "Prismatic Guard", "Mass Blur", "Melgorehn's Aura"])

      expect(ActiveSpell.instance_methods.update_spell_durations.keys[0]).to eq(%[Bravery])
      expect(ActiveSpell.instance_methods.update_spell_durations.values[2]).to eq(%[2023-03-16 18:35:20.205097 -0400])
      expect(ActiveSpell.instance_methods.update_spell_durations.keys[17]).to eq(%[Mage Armor])
    end
  end
end
